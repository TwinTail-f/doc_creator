...
Голый процент покрытия ничего не говорит модели о том, *какой код* не
покрыт. Этот скрипт для каждой непокрытой строки/ветки находит функцию, в
...
Использование:
    ...
    3. Скорми получившийся coverage_gaps.md в LLM с вопросом
       "какие тесты стоит добавить, приоритет — непокрытые raise/except".
       Файл уже содержит код, а не только номера строк — модели не нужно
