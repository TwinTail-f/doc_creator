...
def test_save_failure_does_not_raise(tmp_path: Path) -> None:
    """
    BL-REG-10
    Business Rule: file write error does not interrupt publishing (warning logged).

    Preconditions:
        - data_dir is a read-only directory so write_text() will fail with PermissionError.

    Steps:
        1. Create a read-only directory under tmp_path.
        2. Instantiate PassportPageRegistry pointing to that directory.
        3. Call registry.save(pages_map).

    Expected Result:
        save() swallows the OS-level permission error and does NOT raise any exception.
        Publishing must continue uninterrupted even when the registry cannot be persisted.
    """
    import os
    ...
    registry = PassportPageRegistry(data_dir=readonly_dir)
    pages_map = {
        "openssl": {
            "1.0.0": {
                "page_id": "p-001",
    ...
    try:
        registry.save(pages_map)
        # Reaching this point means save() correctly absorbed the error
    except Exception as exc:
        pytest.fail(
            f"save() must not raise an exception on write error, but got: {exc}"
        )
    finally:
