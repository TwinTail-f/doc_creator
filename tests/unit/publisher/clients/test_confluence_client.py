...
@pytest.mark.contract
def test_get_page_reraises_confluence_error(confluence_client: ConfluenceClient) -> None:
    """get_page пробрасывает ConfluenceError от транспорта без подавления."""
    confluence_client._mock_transport.get_content.side_effect = ConfluenceError("HTTP 404")
    with pytest.raises(ConfluenceError):
        confluence_client.get_page(PAGE_ID)


@pytest.mark.contract
def test_create_page_reraises_confluence_error(confluence_client: ConfluenceClient) -> None:
...
@pytest.mark.contract
def test_get_page_body_returns_body_when_found_in_correct_subtree(
    confluence_client: ConfluenceClient,
) -> None:
    """get_page_body возвращает body, если страница найдена в своём поддереве (parent_id совпадает)."""
    page_data = {
        "id": PAGE_ID,
        "title": PAGE_TITLE,
        "body": {"storage": {"value": "<p>html</p>"}},
...
@pytest.fixture
def confluence_client_page_not_found(confluence_client: ConfluenceClient) -> ConfluenceClient:
    """confluence_client, где find_page ничего не находит, а create_content отвечает успешно."""
    confluence_client._mock_transport.search_content.return_value = []
...
@pytest.fixture
def confluence_client_page_exists(confluence_client: ConfluenceClient) -> ConfluenceClient:
    """confluence_client, где find_page находит существующую страницу под PARENT_ID, а update_content отвечает успешно."""
    existing = {
        "id": PAGE_ID,
        "title": PAGE_TITLE,
        "version": {"number": 3},
...
def test_publish_page_creates_new_page_when_not_exists(
    confluence_client_page_not_found: ConfluenceClient,
) -> None:
    """Если страница не существует, вызывается create_content."""
    confluence_client_page_not_found.publish_page(
        space=SPACE, parent_id=PARENT_ID, title=PAGE_TITLE, body_html=PAGE_BODY
    )
    confluence_client_page_not_found._mock_transport.create_content.assert_called_once()


@pytest.mark.business_logic
def test_publish_page_updates_existing_page_when_exists(
    confluence_client_page_exists: ConfluenceClient,
) -> None:
    """Если страница существует, вызывается update_content."""
    confluence_client_page_exists.publish_page(
        space=SPACE, parent_id=PARENT_ID, title=PAGE_TITLE, body_html=PAGE_BODY
    )
    confluence_client_page_exists._mock_transport.update_content.assert_called_once()
...
def test_publish_page_returns_dict_with_id_version_status(
    ...
    result = confluence_client_page_not_found.publish_page(
        space=SPACE, parent_id=PARENT_ID, title=PAGE_TITLE, body_html=PAGE_BODY
    )
    assert result.id == PAGE_ID
...
@pytest.mark.business_logic
def test_resolve_existing_page_id_returns_id_without_creating_when_page_exists(
    confluence_client: ConfluenceClient,
) -> None:
...
@pytest.mark.business_logic
def test_resolve_then_create_page_when_not_exists(
    confluence_client: ConfluenceClient,
) -> None:
