...
class ManifestParser(BaseDataFetcher[List[Component]]):
    """
    ...
    def _parse_files(self, files: List[Path], excluded: List[str]) -> List[Component]:
        ...
        for filepath in files:
            try:
                ...
                logger.warning('ManifestParser: не удалось прочитать %s: %s', filepath.name, e)
                continue

            name = props.get('name', '')
            if not name:
                logger.debug('ManifestParser: пропуск %s — отсутствует поле "name"', filepath.name)
                continue

            if name in excluded:
                logger.debug('ManifestParser: компонент "%s" исключён', name)
                excluded_count += 1
                continue
        ...
        logger.info('ManifestParser: обработано %d компонентов, исключено %d', parsed_count, excluded_count)
        return components
        ...
        for p_ver in plat_versions:
            ...
            for c_ver in comp_versions:
                ...
                releases.append(Release(
                    ...
                    svace_report=SvaceReport(profile=svace_profile, report_url=''),
                    profile_builds=[
