"""
Резолвер опций Conan: скачивает options.json из репозиториев компонентов.
"""
import json
...
class OptionsResolver(BaseDataFetcher[OptionsMap]):
    """
    ...
    **Не мутирует** входные модели — возвращает ``OptionsMap``.
    ...
    def fetch(self, components: List[Component]) -> OptionsMap:
        ...
        """
        logger.info('OptionsResolver: начинаем сбор options.json…')
        ...
        for comp in components:
            ...
            if not repo_name:
                logger.debug('OptionsResolver: "%s" без git_repo, пропуск', comp.name)
                continue
        ...
        logger.info('OptionsResolver: завершён. Собрано опций для %d релизов.', len(result))
        return result
    ...
    def _fetch_options_for_repo(self, repo_name: str, branch: str) -> Dict:
        ...
        try:
            ...
            logger.warning('OptionsResolver: пропуск репо "%s" (ветка "%s"): %s', repo_name, branch, e)
            return repo_data
    ...
    def _load_single_options_file(
        ...
        try:
            ...
            logger.warning('OptionsResolver: ошибка чтения %s: %s', opt_path, e)
            return
