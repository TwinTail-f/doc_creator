...
def _make_task(comp_name='my_lib', version='1.0.0', channel='stable') -> ConanTask:
    ...
    return ConanTask(
        ...
        comp_name=comp_name, version=version, channel=channel,
        profile_name='linux_x86_64', option_id='1', option_str='shared=True',
...
def _make_conan_graph(comp_name='my_lib', version='1.0.0', channel='stable',
    ...
    node = {
        'name': comp_name, 'ref': ref, 'rrev': 'rev001', 'package_id': package_id,
        'default_options': {'shared': 'False'},
        'options_definitions': {'shared': ['True', 'False']},
        'info': {'settings': {'os': 'Linux', 'arch': 'x86_64'}, 'options': {'shared': 'True'}},
        'prev_timestamp': None,
    }
    if include_patches:
        node['conandata'] = {'patches': {'1.0': [{'patch_file': 'patches/fix_build.patch'}]}}
    if include_deps:
        node['dependencies'] = {
...
class TestConanResultParser:
    ...
    def test_missing_target_node_returns_none(self) -> None:
        result = self.parser.parse(_make_conan_graph(comp_name='other_lib'), _make_task())
    ...
    def test_patches_extracted(self) -> None:
        result = self.parser.parse(_make_conan_graph(include_patches=True), _make_task())
    ...
    def test_default_options_type_detection(self) -> None:
        result = self.parser.parse(_make_conan_graph(), _make_task())
        shared_opt = next((o for o in result.default_options if o['name'] == 'shared'), None)
