...
Для каждой пары укажите либо `*_name` (рекомендуется), либо `*_id`. Если заданы оба —
приоритет у `*_name`.
...
> [!NOTE]
> В отличие от полей конфига (`*_name` / `*_id`), где при совпадении обоих
> побеждает `*_name`, парные CLI-флаги вида `--root-page-name` / `--root-page-id`
> **нельзя указывать одновременно** — команда завершится ошибкой
...
| `--root-page-name "Название"` | Название родительской страницы (переопределяет `release_docs_root_parent_name`). Если содержит пробелы — заключите в кавычки |
| `--root-page-id ID` | ID родительской страницы (переопределяет `release_docs_root_parent_id`). Нельзя указывать вместе с `--root-page-name` |
...
| `--root-page-name "Название"` | Название родительской страницы (переопределяет `profile_docs_root_parent_name`). Если содержит пробелы — заключите в кавычки |
| `--root-page-id ID` | ID родительской страницы (переопределяет `profile_docs_root_parent_id`). Нельзя указывать вместе с `--root-page-name` |
| `--no-passport-links` | Не вставлять ссылки на паспорта компонентов |
